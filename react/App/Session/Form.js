  ...
  render = () =>
    <div className="session-form">
      <form onSubmit={this.submit}>
        ...
        <input type="text" onChange={this.change("email")} value={this.state.email} placeholder="Email" />
        {this.renderErrors("email")}
        <input type="password" onChange={this.change("password")} value={this.state.password} placeholder="Password" />
        {this.renderErrors("password")}
        <button type="submit">{this.props.type}</button>
        <button onClick={this.demo} className="demo">Demo</button>
        <h2>or{" "}
          <Link to={this.oppositeLink()}>
            {this.oppositeType()}
          </Link>
        </h2>
      </form>
